  ...
  if (provider.kind === "cli") {
    ...
    const actionProtocol = [
      ...
      prefs.aiFileToolsEnabled
        ? workspacePath
          ? "To inspect files or request a workspace action, emit exactly one JSON object or array in a fenced `husk-action` block. Use these exact object shapes: {\"kind\":\"workspace.read\",\"path\":\"relative/file\"}; {\"kind\":\"workspace.list\",\"path\":\".\"}; {\"kind\":\"workspace.search\",\"query\":\"text\",\"limit\":10}; {\"kind\":\"workspace.write\",\"path\":\"relative/file\",\"content\":\"...\"}; {\"kind\":\"workspace.edit\",\"path\":\"relative/file\",\"search\":\"...\",\"replace\":\"...\"}; {\"kind\":\"workspace.revertEdit\",\"path\":\"relative/file\"}. The `kind` field appears exactly once and always names the action; never use `file` or `directory` as its value. Paths must be relative to the selected workspace. Reads and lists may complete; edits and overwrites are always reviewable before writing."
