...
- Start a **Task**, then choose **Terminal steps** when the objective needs investigation in the shell. On an SSH terminal, where a local workspace Task is intentionally unavailable, use **Diagnose** for the same supervised command loop. With an API-backed model or signed-in CLI subscription, Husk runs one narrow, observable command at a time in the terminal you selected, waits for the real result, and adapts the next step. Every command and output remain visible; anything outside the diagnostic allowlist, any protected environment, and any state-changing action stops for explicit approval. The model only plans—Husk owns and validates terminal execution.
...
- Open **Tools** in the sidebar for utilities, infrastructure integrations, and local JSON plugins. Custom views require explicit command review, refresh only with opt-in, and offer reviewed, local-only staging for row actions. See [Tools and custom plugins](docs/TOOLS.md) for the format, limits, and safety boundaries.
...
| --- | --- | --- |
| **API-backed model** | Native tool-call loops | Chat, scoped workspace actions, configured MCP integrations, and supervised terminal steps inside a Task. API tool calls enter the Husk Action Broker. |
| **Signed-in CLI subscription** | Using an existing CLI plan | Chat, code questions, terminal help, supervised terminal-step planning, and the same scoped workspace and integration actions without an API key. The CLI returns a validated action proposal; Husk runs it through the same broker. |
...
#### Small AI tools, beside the work

- **AI Peek:** select terminal or editor text and right-click **Explain here** for a compact explanation with links to the selected source lines. **Ask more** prepares a chat draft; **Save to Vault** keeps the explanation and excerpt as a note.
- **Compare this run:** repeat the same command in the same terminal, directory and known host. **Compare with previous** shows a local line diff and real exit codes. **Explain changes** optionally asks AI about the captured differences, with clickable evidence. Captures are bounded and kept only in memory; partial output is labelled. Shell command markers and reliable directory/host information are required, so an uninstrumented SSH session may not offer comparisons.
- **Tweak command:** right-click a selected command or an existing prompt draft and choose **Modify command with AI…**. Describe a change, inspect the original and proposal, then explicitly **Stage in terminal**. Staging requires a verified empty, idle prompt in the same terminal/directory/host and never presses Enter; it will not overwrite existing input. If shell metadata or prompt decoration makes that uncertain, use **Copy** and paste manually after reviewing the destination.
- **Log Highlights:** in **Beautiful Logs**, select lines or filter the output, then choose **Highlights**. Review the frozen sample before **Analyze**. Each finding links to its captured lines, even while live output continues.
- **Find that command:** press **Ctrl+R** in a local shell (or right-click **History…**), switch from the default local search to **Ask AI**, and describe a remembered command. Review the bounded history sample, uncheck anything you do not want sent, then choose **Find matches**. Results are actual saved commands, not generated suggestions, and are staged without execution. Local shell history is not remote-host history; unavailable dates or hosts are labelled as unrecorded. SSH's native Ctrl+R remains available to the remote shell.
- **Selection actions:** selecting terminal text reveals a small **Explain / Review command / Save to Vault** bar. It leaves shell input and copy/paste alone; press **F6** while it is visible to focus its buttons. Review is available for one plain command; larger selections can still be saved locally.
- **Review before running:** right-click a selected command or prompt draft to see local text hints and the captured shell scope. **Explain effects with AI** is a separate explicit request. No command runs, no server inspection is performed, and neither the local hints nor the AI interpretation certify safety or know which resources would be affected.
- **Remember this fix:** when an observed failed command later succeeds in the same verified terminal scope, review the captured successful steps and explicitly confirm/save a fix. Optionally include a bounded error excerpt; successful output is never saved. A future matching command/error in the same directory and host can show **Previous fix found**. Only relevant suggestions occupy a compact terminal strip; saving shows a brief confirmation and removes it. Right-click the terminal → **Saved fixes…** to open the local library (up to 30 fixes / 64 KB), including when AI is disabled or no text is selected. Close with **×** or **Escape** without losing records; there is no permanent library row. Reuse is copy-only and nothing is sent to AI. **Storage & retention** explains local-only storage and no automatic expiry: records remain until deleted or Husk’s app data is cleared. Missing shell markers, unknown scope or partial captures cannot establish a fix.
- **Needs you:** a compact **Review · N** pill in the AI composer counts this chat's pending file edits, integration actions and composer terminal approvals. Choose an item to reveal its existing approval card, not approve it. Ordinary suggestions and Task's internal supervised-step proposals are not counted.
- **Explore output:** in **Beautiful Logs**, choose **Explore** for a frozen, local-only inspector. CSV/TSV and flat JSON arrays can become sortable/filterable tables; compatible numeric columns can become source-linked bar charts. A clickable outline uses actual captured headings. Unsupported, ambiguous or clipped data stays text-only. This does not generate diagrams or send output to a model.

Actions that request AI use the configured provider without attaching a workspace, project memory or unrelated terminal output. Logs and history show the sample before sending; Peek sends the selected excerpt when you choose **Explain here** or **Explain**. Likely credentials block a request (sensitive history entries are excluded), but detection is not exhaustive: review text before sending or saving. No SSH/SFTP workspace is required. Stopping, closing or changing the relevant terminal cancels pending analysis where applicable, and late answers are ignored.
...
- Collect commands using **Add to workflow…** on terminal selections and AI shell-code blocks. Edit in a resizable dock beside the source, collapse without losing the window-local draft, and expand optional inputs when needed. Import/export versioned JSON from the Workflows menu, preview conflicts, and review the exact terminal, directory, and expanded commands before every run. See the [workflow guide](docs/WORKFLOWS.md) for the JSON format and execution boundaries.
